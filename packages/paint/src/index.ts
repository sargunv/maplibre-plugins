...
// compile through the MapLibre style spec so camera expressions and CSS
// colors work like they do for built-in layers, and every change transitions
// the way MapLibre Native transitions plugin paint properties: 300 ms, ease
 ...
 * for one property. Throws on values the style spec rejects. `name` only
 * labels errors.
 */
  ...
  return (zoom) => {
    ...
    if (type === "color") {
      if (!(evaluated instanceof Color))
        return premultiply(spec.default as readonly number[]);
...
export class PaintState<Name extends string> {
  ...
  constructor(
    ...
    for (const name of names) {
      ...
      this.values.set(
        name,
        new PaintValue(name, spec[name].type, compile(spec[name], name, value)),
      );
  ...
  set(
    ...
    const compiled = compile(this.spec[name], name, raw);
